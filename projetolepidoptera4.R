...
# species number ~ temp
model_sp_temp <- lm(richness ~ A_mean_temp, data = sites_data)
model_sp_temp
summary(model_sp_temp)
...
ggplot(data = sites_data) +
  ...
  geom_point() +
  geom_smooth(method = "lm", col = "brown4", shape = 2) +
  theme_bw() +
...
model_ws_alt <- lm(wing_size ~ Altitude1km, data = sites_data)
model_ws_alt
summary(model_ws_alt)
...
# species number ~ altitude
model_sp_alt <- lm(richness ~ Altitude1km, data = sites_data)
model_sp_alt
summary(model_sp_alt)

# plot
ggplot(data = sites_data) +
  aes(x = Altitude1km, y = richness) +
  geom_point()+
  geom_smooth(method = "lm", col = "turquoise3") +
  theme_bw() +
  labs(x = "Altitude (m)", y = "Número de espécies")

# Latitude ~ wing_size
model_lat_ws<- lm(Latitude ~ wing_size, data = sites_data)
model_lat_ws
summary(model_lat_ws)

# plot
ggplot(data = sites_data) +
  aes(x = wing_size, y = Latitude) +
  geom_point()+
  geom_smooth(method = "lm", col = "cornflowerblue") +
  theme_bw() +
  labs(x = "Tamanho da asa (cm)", y = "Latitude")

# Precip ~ wing size
model_ws_pc <- lm(wing_size ~ A_rainfall, data = sites_data)
model_ws_pc
summary(model_ws_pc)

# plot
ggplot(data = sites_data) +
  aes(x = A_rainfall, y = wing_size) +
  geom_point() +
  geom_smooth(method = "lm", col = "steelblue", shape = 2) +
  theme_bw() +
  labs(x = "Preciptação (mm)", y = "Tamanho da asa (cm)")

# Slope ~ wing size
model_ws_slope <- lm(wing_size ~ slope, data = sites_data)
model_ws_slope
summary(model_ws_slope)

# plot
ggplot(data = sites_data) +
  aes(x = slope, y = wing_size) +
  geom_point() +
  geom_smooth(method = "lm", col = "darkgreen", shape = 2) +
  theme_bw() +
  labs(x = "Slope", y = "Tamanho da asa (cm)")

# Homo ~ wing size
model_ws_homo <- lm(wing_size ~ homo, data = sites_data)
model_ws_homo
summary(model_ws_homo)

# plot
ggplot(data = sites_data) +
  aes(x = homo, y = wing_size) +
  geom_point() +
  geom_smooth(method = "lm", col = "orange", shape = 2) +
  theme_bw() +
